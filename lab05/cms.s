...
find_student_by_id__loop__body:
	la	$t1, students					# $t1 = base of students
	mul	$t2, $t0, SIZEOF_STRUCT_STUDENT			# $t2 = i * 8
	add	$t3, $t1, $t2					# $t3 = &students[i]
	lw	$t4, 0($t3)					# $t4 = students[i].id
	beq	$t4, $a0, find_student_by_id__found		# if match, return
...
find_student_by_id__loop__end:
	li	$v0, 0						# return NULL if not found
	j	find_student_by_id__epilogue

find_student_by_id__found:
...
update_student_mark__body:
	...
	beq $t0, $zero, student_is_null
        ...
        sw $t1, STRUCT_STUDENT_MARK_OFFSET($t0)
...
print_report__loop__body:
	...
	lw   $t4, STRUCT_STUDENT_MARK_OFFSET($t3)	# mark = students[i].mark
	li   $t5, UNKNOWN_MARK				# -1
	beq  $t4, $t5, print_unknow
